...
def index():
	...
	if form.validate_on_submit():
		...
		db.session.add(post)
		db.session.commit()
		flash(["Your post in now live!", 'primary'])
...
def register():
	...
	if form.validate_on_submit():
		...
		db.session.add(user)
		db.session.commit()
		flash(["Registration completed successfully!", 'primary'])
		return redirect(url_for('login'))
...
def follow(username):
	...
	if form.validate_on_submit():
		...
		if user is None:
			flash(["User {} not found!".format(username), 'primray'])
			return redirect(url_for('index'))
		elif user == current_user:
			flash(["You can't follow yourself!", 'primary'])
			return redirect(url_for('user'), username=username)
		current_user.follow(user)
		db.session.commit()
		flash(["You are now following user {}!".format(username), 'primary'])
		return redirect(url_for('user', username=username))
	return redirect(url_for('index'))
...
def unfollow(username):
	...
	if form.validate_on_submit():
		user = User.query.filter_by(username=username).first()
		if user is None:
			flash(["User {} not found!".format(username), 'primary'])
			return redirect(url_for('index'))
		elif user == current_user:
			flash(["You can't unfollow yourself!", 'primary'])
			return redirect(url_for('user', username=username))
		current_user.unfollow(user)
		db.session.commit()
		flash(["You are no longer following user {}!".format(username), 'primary'])
		return redirect(url_for('user', username=username))
	return redirect(url_for('index'))
...
def reset_password(token):
	...
	if form.validate_on_submit():
		user.set_password(form.password.data)
		db.session.commit()
		flash(["Your password has changed successfully!", 'primary'])
		return redirect(url_for('login'))
